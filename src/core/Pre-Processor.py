...
# the final dataframe structure will be: u, items, ratings, n_interactions, rho, rho_positive
l_rho = []
l_rho_positive = []
...
bias = 0.7  # the importance given to the "total ratings" bias. The larger the number, the lower the score of items
# with few ratings.
...
for (i, tot_stars, tot_rat) in zip(range(len(popularity_score)), total_stars, total_ratings):
    if tot_rat > 0 and tot_stars > 0:
        popularity_score[i] = tot_stars / tot_rat + utils.normalize(tot_rat,
                                                                    [np.min(total_ratings), np.max(total_ratings)],
                                                                    [0, bias])
    else:
...
# saving the popularity_scores_df to a csv file

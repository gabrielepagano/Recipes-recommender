...
def main():
    ...
    recommended_with_popularity_model = (
        popularity_helper.recommend_items(u_id, topn, prnt=True, verbose=False))

    item_collaborative_helper = itemCollaborativeF.ItemCollaborativeHelper()
    recommended_with_item_collaborative_model = (
        item_collaborative_helper.recommend_items(u_id, topn, prnt=True, verbose=True))
